...
namespace Network
{
    public class PlayersPingList
    {
        public PlayersPingList(float sendPingInterval)
        {
        ...
        public void PingReceived(Player player, int ping)
        {
            int index = _playerPings.FindIndex(x => x.Player == player);
            ...
            out int lowestAveragePing, bool isIncludeLocal = true)
        {
            ...
            for (int pingIndex = 0; pingIndex < _playerPings.Count; pingIndex++)
            {
                ...
                if (isIncludeLocal == false 
                    && playerPing.Player.Equals(PhotonNetwork.LocalPlayer))
                    continue;
